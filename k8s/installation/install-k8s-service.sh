...
#                2021-05-27 merge install-k8s-common.sh & this, because addons
#                           installation scripts removed to deploy by manual
...
function deploy_k8s_apiServer()
{
  ...
  sed -i "s#\${APISERVER_COUNT}#${#MASTERS[@]}#g" ${TMP_DIR}/kube-apiserver.service
...
function init_env_common()
{
  ...
  for host in ${WORKERS[@]}; do
    ssh ${USER}@${host} "[ sudo rm -fr /tmp/k8s-setup.*; \
      mkdir -p ${TMP_DIR}"
  done
}
