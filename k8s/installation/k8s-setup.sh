...
EOF
  ...
  sudo chown root:root /opt/ssl/kube-proxy-csr.json
  ...
  template:
    ...
    spec:
      ...
      initContainers:
        ...
        - name: install-cni
          image: ${REGISTRY}/calico/cni:v3.15.1
        ...
        - name: flexvol-driver
          image: ${REGISTRY}/calico/pod2daemon-flexvol:v3.15.1
          volumeMounts:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: calico-kube-controllers
          image: ${REGISTRY}/calico/kube-controllers:v3.15.1
          env:
            # The location of the etcd cluster.
            - name: ETCD_ENDPOINTS
              valueFrom:
...
EOF
  ...
  echo "  >> 6.1.7 import docker images (skip)"
...
unused()
{
  ...
  local -r images=($(docker images | grep '^gcr\.io' |  awk '{print $1}' | sed "s:/: :g"|awk '{print $3}'))
  for imageName in ${images[@]}
  do
    tag=$(docker images | grep "${imageName}" | awk '{print $1":"$2}')
    echo "tag = ${tag}"
    docker save ${tag} -o "/mnt/rw/docker-image/gcr.io/google_containers/${imageName}.docker.tar"
  done
}
